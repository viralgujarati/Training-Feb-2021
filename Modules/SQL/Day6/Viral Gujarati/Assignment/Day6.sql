...
/*Write a query to find the names (first_name, last_name) and salary of the employees who earn a salary that is higher than the salary of 
...
SELECT Salary FROM 
(SELECT Salary, DENSE_RANK() OVER (ORDER BY Salary DESC) 'DenseRank' FROM Employees) AS RankTbl
WHERE DenseRank=5

/*Write a query to find the 4th minimum salary in the employees table. */
SELECT DISTINCT Salary FROM 
(SELECT Salary, DENSE_RANK() OVER (ORDER BY Salary) 'DenseRank' FROM Employees) AS RankTbl
WHERE DenseRank=4
...
FROM Locations AS l JOIN Countries AS c
...
FROM Employees AS e JOIN Departments as d
ON e.DepartmentID = d.DepartmentID JOIN Locations AS l
ON l.LocationID = d.LocationID
WHERE l.City='London'

/*Write a query to find the employee id, name (last_name) along with their manager_id, manager name (last_name). */
SELECT e1.EmployeeID, e1.LastName AS 'EmployeeName', e2.EmployeeID AS 'ManagerId', e2.LastName AS 'ManagerName'
...
SELECT d.DepartmentName, COUNT(e.DepartmentID) AS 'NoOfEmployees'
FROM Employees AS e JOIN Departments as d
ON e.DepartmentID = d.DepartmentID
...
SELECT EmployeeID, JobID, DATEDIFF(DAY, StartDate, EndDate) AS 'NoOfDays'
FROM JobHistory
...
FROM Employees AS e JOIN Departments AS d
ON d.ManagerID = e.EmployeeID JOIN Locations AS l
ON l.LocationID = d.LocationID
...
SELECT j.*
FROM JobHistory AS j JOIN Employees AS e
